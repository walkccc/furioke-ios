...
final class SpotifyAdapter: NSObject, MusicSource {
  ...
  func connect() async -> Result<Void, MusicError> {
    // The plain Connect button resumes the user's last context.
    await connect(playing: "")
  }

  /// The shared connect handshake. `uri` is the track to start once Spotify
  /// authorizes: the empty string resumes the user's last context (the Connect
  /// button), while an explicit track URI both primes the session *and* starts
  /// the tapped song — so auto-connect-then-play is a single app switch.
