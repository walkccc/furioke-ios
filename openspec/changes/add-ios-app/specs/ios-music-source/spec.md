...
the web `[[music-source]]` adapter contract. The protocol SHALL expose:
`provider` id, `requiresAccount`, `supportsRepeat`, `getConnection`, optional
`getAccount`, an `updates` async stream of playback snapshots, `connect`,
`disconnect`, `control`, `playTrack`, and `resolveTracks`. Feature code
(NowPlaying, Library, Search) SHALL depend only on this protocol, never on
`SPTAppRemote`, `MusicKit`, or YouTube HTTP types directly.
...
A `SpotifyiOSAdapter` SHALL implement `MusicSource` using Spotify's iOS SDK
