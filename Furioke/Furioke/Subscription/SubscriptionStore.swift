...
final class SubscriptionStore {
  ...
  ///
  /// Gated on a permanent account first: Plus is account-bound, but
  /// `localEntitled` is derived from the device's Apple-ID transactions, which
  /// outlive a sign-out and aren't tied to the Furioke account. Without this
  /// guard a guest on a device whose Apple ID once bought Plus would read as
  /// "Active" — `refreshEntitlement()` re-derives `localEntitled` as true from
  /// `currentEntitlements` even after `clearForSignOut()`. The backend half
  /// already zeroes `backendEntitled` for guests; this is the matching local
  /// rule, mirroring the server, which never grants a guest Plus.
  var isPlus: Bool {
    guard !auth.isGuest else { return false }
    return localEntitled || backendEntitled
  }
  ...
  /// Drop the entitlement to its signed-out baseline. Wired to the auth
  /// sign-out teardown so a guest never inherits the prior account's Plus: the
  /// backend flag is per-account, and `localEntitled` is recomputed from the
  /// device's transactions on the next refresh. Without this, the in-memory
  /// `backendEntitled` would keep showing "Active" until the app was killed.
  func clearForSignOut() {
    backendEntitled = false
    localEntitled = false
  }
  ...
  ///
  /// A guest is the exception: Plus is reserved for a permanent account, so a
  /// guest holds no backend entitlement by definition. The backend can't be
  /// asked without a token (the read would return nil and *keep* the current
  /// value), so a value carried over from a prior signed-in session would never
  /// clear. Drop it directly instead.
