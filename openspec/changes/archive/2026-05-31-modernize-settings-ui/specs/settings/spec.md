...
`ReadingOverridesView`, rendered as a tappable card row with a leading glyph and
a trailing chevron affordance. It SHALL present a Sign Out action as a distinct,
clearly-destructive full-width control that invokes `AuthService.signOut()`. The
destinations and actions SHALL be unchanged from the existing behavior; only
