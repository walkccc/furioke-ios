...
flow. The Save button SHALL be disabled when no track is playing or when the
track is already saved (in which case the button SHALL show a "Saved" state with
a tap-to-open-library affordance).
...
translation cache when available. Cache entries SHALL be keyed by
`(song id, language, model version)`.
