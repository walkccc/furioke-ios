...
NowPlaying is the Furioke iOS app's primary playback surface, delivered as the
expanded state of the persistent mini-player and driving the active music
provider directly while following externally-initiated playback in companion
mode. It presents transport controls, a draggable position scrubber, and a queue
/ up-next surface, and synchronizes a lyric view that highlights the active
line, supports tap-to-seek, save-to-library, and a cached translation toggle. It
integrates with the system via `MPNowPlayingInfoCenter`,
`MPRemoteCommandCenter`, and a CarPlay Now-Playing template, and renders the
playing track's album art as an ambient, blurred backdrop behind the lyrics.
...
persistent `MiniPlayer` owned by `AppShell`. NowPlaying SHALL NOT be a peer tab
destination. The mini-player and the sheet SHALL share a `matchedGeometryEffect`
namespace owned by `AppShell` so the artwork, title, and artist morph in place
between collapsed and expanded states rather than cross-fading.
...
next — using the shared `TransportButton` primitive. Each button SHALL delegate
to the active adapter's `control` function. Controls that the active adapter or
the current playback context cannot service SHALL be visually disabled (rendered
at ~35% opacity) rather than hidden, so the layout is stable. Each tap SHALL
animate via `Motion.pop` with the `.bounce` SymbolEffect choreography.
...
currently-playing track into the user's `songs` table via the existing save
flow. Saving to the Library is reserved for a permanent (non-anonymous) account:
when a guest taps **Save**, the app SHALL present the in-app sign-in prompt
instead of inserting a row, and SHALL perform the save only after the guest
upgrades to a permanent account. For a permanent account, the Save button SHALL
be disabled when no track is playing or when the track is already saved (in
which case the button SHALL show a "Saved" state with a tap-to-open-library
affordance).

#### Scenario: Save inserts a saved-song row

- **WHEN** a permanent-account user taps **Save** while a Spotify or Apple Music
  track is playing
...
#### Scenario: Guest tapping Save is prompted to sign in

- **WHEN** a guest (anonymous) session taps **Save** while a track is playing
- **THEN** the in-app sign-in prompt is presented and no `songs` row is
  inserted; after the guest upgrades to a permanent account the save can proceed

#### Scenario: Save state reflects already-saved

- **WHEN** the currently-playing track is already in a permanent-account user's
  `songs` table
...
whole-body translations of each lyric line. Whole-body translation is reserved
for a permanent (non-anonymous) account — the backend `/api/translate` route
rejects anonymous callers — so when a guest enables Translation the app SHALL
present the in-app sign-in prompt instead of fetching, and SHALL enable
Translation only after the guest upgrades. For a permanent account, translations
SHALL be fetched from `/api/translate` when not in cache and SHALL be served
from the local SwiftData translation cache when available. Cache entries SHALL
be keyed by `(song id, language, model version)`.

#### Scenario: Guest enabling Translation is prompted to sign in

- **WHEN** a guest (anonymous) session enables the Translation toggle
- **THEN** the in-app sign-in prompt is presented, no `/api/translate` call is
  made, and the toggle does not stay on until the guest upgrades to a permanent
  account

#### Scenario: Toggle on, cache hit

- **WHEN** a permanent-account user enables Translation for a song whose
  translation is in the local cache
...
- **WHEN** a permanent-account user enables Translation for a song whose
  translation is not in the cache and the device is online
...
- **WHEN** a permanent-account user enables Translation for a song whose
  translation is not cached and the device is offline
...
ambient, blurred backdrop behind the lyrics, mirroring the web app's now-playing
