...
/// vs. session-only). Defaults to remembering.
...
final class NowPlayingState {
  ...
  private func apply(_ event: CacheLoad<LyricFetchResult>) async {
    switch event {
    case let .value(result):
      ...
      do {
        ...
        lines = annotated
        furiganaLoading = false
        // Translate the freshly-loaded body when the toggle is already on (the
        // user enabled it before lyrics arrived, or a background revalidation
        // changed the body).
  ...
  /// draft starts from the token's current reading and the **Remember this
  /// reading** toggle defaults on.
  func beginEditing(surface: String, reading: String) {
  ...
  /// rows and the `reading_overrides.user_id` column; nil when signed out.
