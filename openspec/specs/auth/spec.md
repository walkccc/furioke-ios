...
Keychain, clear in-memory session state, purge per-user SwiftData entities, and
transition the app to the sign-in surface. Provider connections (Spotify access
token, MusicKit authorization) SHALL also be revoked locally so a different
signed-in user on the same device does not inherit them.
