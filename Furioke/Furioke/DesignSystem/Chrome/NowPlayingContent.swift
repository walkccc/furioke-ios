...
struct NowPlayingContent<Lyrics: View, Timeline: View, Video: View>: View {
  ...
  /// nothing to surface.
  let translationNotice: LocalizedStringKey?
  ...
  let playbackNotice: LocalizedStringKey?
  ...
  private var topBanners: some View {
    VStack(spacing: Spacing.s) {
      ...
      if let translationNotice {
        Toast(text: translationNotice, kind: .icon("translate"))
          .transition(.move(edge: .top).combined(with: .opacity))
      }
  ...
  private var optionsMenu: some View {
    ...
    .buttonStyle(.plain)
    .glassEffect(Materials.controlTier.glass(active: showRomaji || showTranslation), in: Circle())
    ...
    VStack(alignment: .leading, spacing: Spacing.m) {
      HStack(spacing: Spacing.m) {
        ...
        VStack(alignment: .leading, spacing: 2) {
          // The placeholder is a literal so it localizes; a real title is verbatim.
          (title.isEmpty ? Text("Not playing") : Text(verbatim: title))
